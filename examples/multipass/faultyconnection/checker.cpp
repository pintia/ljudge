...
int main(int argc, char* argv[]) {
	...
	std::ifstream user_output("user_output");
    OutputValidator v(argc, argv, user_output);
    v.set_WA_handler([&]() {
        std::ofstream judgemessage{std::filesystem::path("feedback_dir") / "judgemessage.txt"};
        judgemessage << "WA" << std::endl;
    });

    std::filesystem::path state_file    = std::filesystem::path("feedback_dir") / "state.txt";
    std::filesystem::path nextpath_file = std::filesystem::path("feedback_dir") / "nextpass.in";
